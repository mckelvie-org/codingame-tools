...
Three things in this project are produced by tools rather than written by hand, and they are
governed differently on purpose.

| | the documentation site | `codingame_tools/contribution_manager/assets/cover-placeholder.png` |
|---|---|---|
| Built by | `bin/gen-docs` | `bin/gen-default-cover-image` |
| Derived from | the `cg` argparse tree and the package's docstrings | `scripts/gen_cover_placeholder.py` alone |
| Committed? | **no** | **yes**, and treated as source |
| Can it go stale? | no -- it does not exist between builds | no |

**Nothing about the documentation is committed.** Both generated parts of the site are produced
during the build, into mkdocs' virtual file tree:

- the **API reference**, from the package's docstrings (`scripts/gen_api_pages.py`)
- the **CLI command reference**, from the `cg` argparse tree (`scripts/gen_cli_pages.py`, rendering
  via `scripts/gen_cli_docs.py`)

The CLI reference used to be committed and regenerated by hand. That made sense only while the
README linked readers at those Markdown files on GitHub; once it started linking at the rendered
site, the committed copy was pure downside -- nothing in CI checked it was current, so a changed
help string could publish a site documenting the previous one. Generating it during the build makes
that unrepresentable instead of merely discouraged, and drops 32 generated files out of PR diffs.

The hand-written guides under `doc/` *are* source, and are the same files the site renders.

**`bin/gen-docs` builds the site** with `--strict`, so an unresolved cross-reference fails locally
exactly as it does in CI. `bin/gen-docs --serve` runs a live-reloading server instead, which is what
you want while writing docs.
...
is the point -- a bare `properdocs serve` outlives the tab you were reading and squats on a port
...
`bin/docs -q` skips the rebuild and serves the last build, which is what you want when reading
rather than writing.

`cg doc` is the user-facing counterpart, and deliberately does *not* share this directory. It opens
the published docs for the installed version, or, inside a checkout, builds that tree's docs into a
per-user cache (`~/.cache/codingame/cg/docs/<checkout>-<hash>`) and serves from there. The split is
by which tool it is, not by what happens to be writable: `cg doc` is something a user of the package
runs from anywhere on the system, so it treats a source tree as read-only on principle. `site/`
belongs to the contributor tools above -- and to `mike`, which deploys from it.
